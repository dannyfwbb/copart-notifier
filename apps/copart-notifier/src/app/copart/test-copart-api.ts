 ...
 *
 * This script can be run directly with ts-node to test the Copart API integration
 * without running the entire application.
 *
 * Usage:
 * npx ts-node -r tsconfig-paths/register apps/copart-notifier/src/app/copart/test-copart-api.ts
 */
...
async function testCopartApi() {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      console.error(
        'API returned a redirect. This might indicate authentication issues.'
      );
      return;
    }
    ...
    listings.slice(0, 3).forEach((lot, index) => {
      ...
      console.log(
        `- Auction Date: ${
          lot.ad ? new Date(lot.ad).toLocaleDateString() : 'N/A'
        }`
      );
